...
export const keepkeyConfig = path.join(homedir, ".keepkey", "keepkey.json");
export const configPath = path.join(homedir, ".keepkey", "keepkey.json");
export const seedPath = path.join(
  homedir,
  ".keepkey",
  "wallet_data/wallet.seed"
);
export const seedDir = path.join(homedir, ".keepkey", "wallet_data");
export const keepkeyPath = path.join(homedir, ".keepkey");
export const modelDir = path.join(homedir, ".keepkey", "models");
export const backtestDir = path.join(homedir, ".keepkey", "backtest");
export const appDir = path.join(homedir, '.keepkey', 'apps')
export const logDir = path.join(keepkeyPath, "log");
...
export async function innitConfig(languageSelected: string) {
  ...
  try {
    ...
    // //console.log(tag, "keepkeyPath: ", keepkeyPath);
    // //console.log(tag, "seedDir: ", seedDir);

    let isCreated = await mkdirp(keepkeyPath);
    ...
    fs.writeFileSync(keepkeyConfig, JSON.stringify(config));
  } catch (e) {
...
export function checkConfigs() {
  ...
  let fileFound = fs.existsSync(keepkeyConfig) ? true : false;
  if (fileFound) {
